...
# ================== BOT CONFIG (NO .env REQUIRED) ==================

TOKEN = "7646276505:AAFMnuKHBAE21HQz_NAr538ZyoUst6em9N4"

OWNER_ID = 6321618547        # <-- তোমার Telegram numeric ID
ADMIN_ID = 6321618547        # <-- চাইলে আলাদা admin ID দাও

YOUR_USERNAME = "@RASHIK_69"   # <-- তোমার Telegram username
UPDATE_CHANNEL = "https://t.me/TrickHubBD_2nD"  # <-- Update channel link
...
async def cmd_restart(message: types.Message):
    if message.from_user.id != OWNER_ID:
        await message.answer("❌ Only owner can restart the bot!")
        return

    await message.answer(
        "🔄 <b>Restarting bot...</b>\n\n"
        "🛑 Stopping all running scripts...\n"
        "⏳ Please wait...",
        parse_mode="HTML"
    )

    # Stop all running scripts safely
    for script_key, info in list(bot_scripts.items()):
        try:
            ...
            logger.error(f"Error stopping script during restart: {e}")
...
async def web_server():
    ...
    async def handle(request):
        return web.Response(text="🚀 Advanced File Host Bot - Running!")
